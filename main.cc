 ...
 *    3.1: the standard AT-1 model using the quadratic degradate function
 ...
 *    3.2: the cohesive AT-1 model using a quasi-linear or quasi-quadratic
 *         rational degradation function
 *         (this model is based on the cohesive crack)
 *         ref: Geelen et al. (2019) "A phase-field formulation
 *              for dynamic cohesive fracture"
 *    3.3: the phase-field cohesive zone model (PFCZM )using a general form of
 *         rational degration function
 *         (this model is based on the cohesive crack)
 *         ref: Jian-ying Wu (2019) "A unified phase-field theory for the
 *              mechanics of damage and quasi-brittle failure"
 */

/* A monolithic scheme based on the L-BFGS method and the gradient projection
 * method to solve the phase-field crack problem
 * 1. The phase-field method treats the phasefield irreversibility using the
 * gradient projection method. During a load step [t_n, t_n+1], let d_n
 * represent the phasefield at the beginning of the load step (known), then the
 * inequality constraints d_n <= d_n+1 <= 1.0 are treated as box constraints.
 * 2. Various direct and iterative linear solvers are designed to improve the
 * wall-clock run time.
...
namespace PhaseField
{
  ...
  usr_Jacobi_preconditioner::usr_Jacobi_preconditioner(
  ...
  usr_sparseLU_preconditioner::usr_sparseLU_preconditioner(
      const SparseDirectUMFPACK &matrix_factorization)
  ...
  {
  public:
    usr_sparseILU_preconditioner(
        const SparseILU<double> &ILU_factorization_disp,
  ...
  usr_sparseILU_preconditioner::usr_sparseILU_preconditioner(
      const SparseILU<double> &ILU_factorization_disp,
      const SparseILU<double> &ILU_factorization_phasefield)
  ...
  // Various phase-field degradation function
                              ...
                              const std::string &model_name)
  {
    ...
    if (model_name == "AT2" || model_name == "AT1-Griffith")
      value = (1.0 - d) * (1.0 - d);
  ...
  // Various phase-field geometric function
                                             ...
                                             const std::string &model_name)
  {
    ...
    if (model_name == "AT2")
      value = d * d;
    else if (model_name == "AT1-Griffith" || model_name == "AT1-Cohesive")
      value = d;
  ...
  namespace Parameters
  {
    ...
    void Scenario::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Scenario");
      {
        ...
        prm.declare_entry("Nonlinear solver type", "LBFGSB",
                          Patterns::Selection("LBFGS|LBFGSB"),
                          "Type of solver used to solve the nonlinear system");

        prm.declare_entry("Line search type", "GradientBased",
        ...
        prm.declare_entry("Preconditioner type for CG", "ILU",
                          Patterns::Selection("None|Jacobi|LU|ILU"),
                          "Type of preconditioner used to solve the linear system");

        prm.declare_entry("CG tolerance", "1.0e-6", Patterns::Double(0.0),
    ...
    void NonlinearSolver::declare_parameters(ParameterHandler &prm)
    {
      ...
      {
        prm.declare_entry("Max iterations BFGS", "20", Patterns::Integer(0),
  ...
  private:
    ...
    //                          or 3 (active constraint lower/upper bound)
    ...
    struct Errors
    {
      Errors() : m_norm(1.0), m_u(1.0), m_d(1.0) {}
    ...
    void
    assemble_system_rhs_BFGS_parallel(const BlockVector<double> &solution_old,
                                      BlockVector<double> &system_rhs);
    ...
    double line_search_stepsize_gradient_based(
    ...
    double line_search_zoom_strong_wolfe(
        ...
        double phi_high_prime, double alpha_high, double phi_0,
        double phi_0_prime, const BlockVector<double> &BFGS_p_vector, double c1,
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::zT_B0_z(
      const BlockVector<double> &z, BlockSparseMatrix<double> &B0_matrix)
  {
    ...
    for (unsigned int i = 0; i < z.block(m_u_dof).size(); ++i)
    {
      if (z.block(m_u_dof)[i] < 0)
      {
        for (auto itr = B0_matrix.block(m_u_dof, m_u_dof).begin(i);
             itr != B0_matrix.block(m_u_dof, m_u_dof).end(i); ++itr)
        {
    ...
    for (unsigned int i = 0; i < z.block(m_d_dof).size(); ++i)
    {
      if (z.block(m_d_dof)[i] < 0)
      {
        for (auto itr = B0_matrix.block(m_d_dof, m_d_dof).begin(i);
             itr != B0_matrix.block(m_d_dof, m_d_dof).end(i); ++itr)
        {
          if (itr->column() != itr->row())
          {
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::z_x_vector(
      const BlockVector<double> &z, const BlockVector<double> &src_vector,
      BlockVector<double> &target_vector)
  {
    ...
    for (unsigned int i = 0; i < z.block(m_u_dof).size(); ++i)
    {
      if (z.block(m_u_dof)[i] > 0)
      {
        target_vector.block(m_u_dof)[i] =
            src_vector.block(m_u_dof)[target_vector_index];
        ++target_vector_index;
      }
      else
    ...
    for (unsigned int i = 0; i < z.block(m_d_dof).size(); ++i)
    {
      if (z.block(m_d_dof)[i] > 0)
      {
        target_vector.block(m_d_dof)[i] =
            src_vector.block(m_d_dof)[target_vector_index];
        ++target_vector_index;
      }
      else
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::zT_x_vector(
      const BlockVector<double> &z, const BlockVector<double> &src_vector,
      BlockVector<double> &target_vector)
  {
    // We assume that the dimensions of all the block matrices are correct
    ...
    for (unsigned int i = 0; i < z.block(m_u_dof).size(); ++i)
    {
      if (z.block(m_u_dof)[i] > 0)
      {
        target_vector.block(m_u_dof)[target_vector_index] =
            src_vector.block(m_u_dof)[i];
        ++target_vector_index;
      }
    }
    ...
    for (unsigned int i = 0; i < z.block(m_d_dof).size(); ++i)
    {
      if (z.block(m_d_dof)[i] > 0)
      {
        target_vector.block(m_d_dof)[target_vector_index] =
            src_vector.block(m_d_dof)[i];
        ++target_vector_index;
      }
    }
  }
  ...
  template <int dim>
  double PhaseFieldMonolithicSolve<dim>::ebT_x_B0_x_v(
      const unsigned int b, const BlockSparseMatrix<double> &B0_matrix,
      const BlockVector<double> &v)
  {
    double row_sum = 0.0;
    for (auto itr = B0_matrix.block(m_d_dof, m_d_dof).begin(b);
         itr != B0_matrix.block(m_d_dof, m_d_dof).end(b); ++itr)
    {
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::point_projection(
    ...
    for (unsigned int i = 0; i < solution_phasefield_total.size(); ++i)
    {
      ...
      if (solution_phasefield_total[i] > upper_limit)
        solution_delta.block(m_d_dof)[i] =
            upper_limit - m_solution.block(m_d_dof)[i];
    }
  }
  ...
  std::priority_queue<std::pair<double, unsigned int>,
                      std::vector<std::pair<double, unsigned int>>,
                      std::greater<std::pair<double, unsigned int>>>
  PhaseFieldMonolithicSolve<dim>::calculate_break_points(
      const BlockVector<double> &solution_delta,
      const BlockVector<double> &gradient_g, BlockVector<double> &gradient_d)
  {
    ...
    for (unsigned int i = 0; i < m_dofs_per_block[m_d_dof]; ++i)
    {
      ...
      // AssertThrow(t >= 0, ExcMessage("Break point has to be a non-negative t
      // value"));
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::get_error_residual_LBFGSB(
      Errors &error_residual, const BlockVector<double> &solution_delta)
  {
    // We use L_2 norm
    BlockVector<double> error_res(m_dofs_per_block);
    ...
    for (unsigned int i = 0; i < m_dofs_per_block[m_d_dof]; ++i)
    {
      ...
      {
        trial_solution =
            solution_phasefield_total(i) - m_system_rhs.block(m_d_dof)[i];

        if (trial_solution <= m_solution.block(m_d_dof)[i])
          error_res.block(m_d_dof)[i] =
              m_solution.block(m_d_dof)[i] - solution_phasefield_total(i);
        else if (trial_solution >= upper_limit)
          error_res.block(m_d_dof)[i] =
              upper_limit - solution_phasefield_total(i);
        else
      ...
      const std::string &data_file, const unsigned int total_material_regions)
  {
    ...
    // add several material parameters for non AT-2 models
    ...
    if (myfile.is_open())
    {
             ...
             p >> a2 >> a3)
      {
        ...
        else if (m_parameters.m_phasefield_name == "AT1-Griffith")
        {
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::setup_qph()
  {
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::update_qph_incremental(
      const BlockVector<double> &solution_delta,
      const BlockVector<double> &solution_old)
  {
  ...
  template <int dim> struct PhaseFieldMonolithicSolve<dim>::PerTaskData_UQPH
  {
    void reset() {}
  };

  template <int dim> struct PhaseFieldMonolithicSolve<dim>::ScratchData_UQPH
  {
      ...
      ScratchData_UQPH &scratch, PerTaskData_UQPH & /*data*/)
  {
    ...
    scratch.m_fe_values[m_d_fe].get_function_values(
        scratch.m_solution_previous_step,
  ...
  template <int dim> struct PhaseFieldMonolithicSolve<dim>::PerTaskData_ASM
  {
  ...
  template <int dim> struct PhaseFieldMonolithicSolve<dim>::ScratchData_ASM
  {
    FEValues<dim> m_fe_values;
  ...
  PhaseFieldMonolithicSolve<dim>::PhaseFieldMonolithicSolve(
      ...
      : m_parameters(input_file),
        ...
        m_d_fe(m_d_component), m_dofs_per_block(m_n_blocks),
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid()
  {
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_1()
  {
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_2()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_3()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_4()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
      ...
      while (initiation_point_refine_unfinished)
      {
        ...
        for (const auto &cell : m_triangulation.active_cell_iterators())
        {
          if ((std::fabs(cell->center()[0] - 0.5) < 0.025 &&
              ...
              // we also need to refine the top edge, since there might be a
              // conflict between inhomogeneous boundary conditions and the
              // hanging-node constraints at the top edge
              || (cell->center()[1] > 0.975))
          {
            material_id = cell->material_id();
            length_scale = m_material_data[material_id][2];
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_5()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
    ...
    if (m_parameters.m_refinement_strategy == "pre-refine")
    {
      for (const auto &cell : m_triangulation.active_cell_iterators())
      {
        if (std::fabs(cell->center()[0] - 4.0) < 0.075 && cell->center()[1] < 1.6)
        {
          cell->set_refine_flag();
        }
      }
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_6()
  {
    ...
    GridGenerator::hyper_shell(tria_outer, Point<dim>(), 0.5, std::sqrt(dim),
                               2 * dim);
    ...
    GridGenerator::create_triangulation_with_removed_cells(
        tmp_triangulation, cells_to_remove, m_triangulation);
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_7()
  {
    AssertThrow(dim == 3, ExcMessage("The dimension has to be 3D!"));
    ...
    Triangulation<dim> tria_outer;
    GridGenerator::hyper_shell(tria_outer, Point<dim>(), 0.49,
                               std::sqrt(dim) * 0.5, 2 * dim);

    Triangulation<dim> cube1;
    GridGenerator::hyper_rectangle(cube1, Point<dim>(0, 0, 0.5),
                                   Point<dim>(1, 1, 1.5));
    Triangulation<dim> cube2;
    GridGenerator::hyper_rectangle(cube2, Point<dim>(0, 0.5, -0.5),
                                   Point<dim>(1, 1.5, 0.5));
    Triangulation<dim> cube3;
    GridGenerator::hyper_rectangle(cube3, Point<dim>(0.5, -0.5, -0.5),
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_8()
  {
    AssertThrow(dim == 3, ExcMessage("The dimension has to be 3D!"));
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_9()
  {
    AssertThrow(dim == 2, ExcMessage("The dimension has to be 2D!"));
    ...
    if (m_parameters.m_refinement_strategy == "pre-refine")
    {
      ...
      for (unsigned int i = 0; i < m_parameters.m_local_prerefine_times; i++)
      {
        for (const auto &cell : m_triangulation.active_cell_iterators())
        {
          if ((cell->center()[1] > 242.0) && (cell->center()[1] < 360.0) &&
              (cell->center()[0] < 258.0))
          {
            material_id = cell->material_id();
            length_scale = m_material_data[material_id][2];
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_10()
  {
    AssertThrow(dim == 3, ExcMessage("The dimension has to be 3D!"));
    ...
    GridGenerator::extrude_triangulation(triangulation_2d, n_layer, thickness,
                                         m_triangulation);
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_11()
  {
    AssertThrow(dim == 3, ExcMessage("The dimension has to be 3D!"));
    ...
    GridGenerator::subdivided_hyper_rectangle(triangulation_2d, repetitions,
                                              point1, point2);
    ...
    GridGenerator::extrude_triangulation(triangulation_2d, n_layer, height,
                                         tmp_triangulation);
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_12()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::make_grid_case_13()
  {
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
    m_logfile << "\t\t\t1-D bar with defect" << std::endl;
    for (unsigned int i = 0; i < 80; ++i)
      m_logfile << "*";
    m_logfile << std::endl;
    ...
    GridGenerator::subdivided_hyper_rectangle(tmp_triangulation, repetitions,
                                              Point<dim>(0.0, 0.0),
                                              Point<dim>(length, width));
    ...
    for (const auto &cell : tmp_triangulation.active_cell_iterators())
    {
      if (std::fabs(cell->center()[0] - length / 2) < h_size &&
          (cell->center()[1] < h_size || cell->center()[1] > width - h_size))
      {
        cells_to_remove.insert(cell);
      }
    }
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::setup_system()
  {
    ...
    m_logfile << "\t\tTriangulation:"
              ...
              << m_dofs_per_block[m_d_dof] << std::endl;
    ...
    m_tangent_matrix.clear();
    {
      ...
      DoFTools::make_sparsity_pattern(m_dof_handler, coupling, dsp, m_constraints,
                                      false);
  ...
  void PhaseFieldMonolithicSolve<dim>::make_constraints(const unsigned int it_nr)
  {
    ...
    if (apply_dirichlet_bc)
    {
      ...
      else if (m_parameters.m_scenario == 5)
      {
        typename Triangulation<dim>::active_vertex_iterator vertex_itr;
        vertex_itr = m_triangulation.begin_active_vertex();
        std::vector<types::global_dof_index> node_bottomleft(
            m_fe.dofs_per_vertex);
        std::vector<types::global_dof_index> node_bottomright(
            m_fe.dofs_per_vertex);
        ...
        for (; vertex_itr != m_triangulation.end_vertex(); ++vertex_itr)
        {
          ...
          {
            node_bottomleft =
                usr_utilities::get_vertex_dofs(vertex_itr, m_dof_handler);
          }
        ...
        m_constraints.set_inhomogeneity(node_topcenter[1],
                                        disp_magnitude * time_inc);
      }
               ...
               m_parameters.m_scenario == 8)
      {
        ...
        VectorTools::interpolate_boundary_values(
            m_dof_handler, x0_surface,
            Functions::ZeroFunction<dim>(m_n_components), m_constraints,
            m_fe.component_mask(x_displacement));
        const int y0_surface = 1;
        VectorTools::interpolate_boundary_values(
            m_dof_handler, y0_surface,
            Functions::ZeroFunction<dim>(m_n_components), m_constraints,
            m_fe.component_mask(y_displacement));
        const int z0_surface = 2;
        VectorTools::interpolate_boundary_values(
            m_dof_handler, z0_surface,
            Functions::ZeroFunction<dim>(m_n_components), m_constraints,
        ...
        VectorTools::interpolate_boundary_values(
            m_dof_handler, z1_surface,
            Functions::ConstantFunction<dim>(disp_magnitude * time_inc,
                                             m_n_components),
            m_constraints, m_fe.component_mask(z_displacement));
      }
      ...
      else if (m_parameters.m_scenario == 11)
      {
        ...
        VectorTools::interpolate_boundary_values(
            m_dof_handler, boundary_id_right_surface,
            Functions::ZeroFunction<dim>(m_n_components), m_constraints,
            m_fe.component_mask(displacements));
      ...
      else if (m_parameters.m_scenario == 13)
      {
        ...
        VectorTools::interpolate_boundary_values(
            m_dof_handler, boundary_id_bottom_surfaces,
            Functions::ZeroFunction<dim>(m_n_components), m_constraints,
            m_fe.component_mask(y_displacement));

        const int boundary_id_top_surfaces = 3;
        VectorTools::interpolate_boundary_values(
            m_dof_handler, boundary_id_top_surfaces,
            Functions::ZeroFunction<dim>(m_n_components), m_constraints,
            m_fe.component_mask(y_displacement));
      }
      else
      ...
      const BlockVector<double> &solution_old)
  {
    ...
    auto copier = [this](const PerTaskData_ASM &data) {
      this->m_constraints.distribute_local_to_global(
          data.m_cell_matrix, data.m_local_dof_indices, m_tangent_matrix);
    };
  ...
  void PhaseFieldMonolithicSolve<dim>::assemble_system_rhs_BFGS_parallel(
      const BlockVector<double> &solution_old, BlockVector<double> &system_rhs)
  {
    ...
    auto copier = [this, &system_rhs](const PerTaskData_ASM_RHS_BFGS &data) {
      this->m_constraints.distribute_local_to_global(
          data.m_cell_rhs, data.m_local_dof_indices, system_rhs);
    };
      ...
      const BlockVector<double> &solution_old, BlockVector<double> &system_rhs)
  {
    ...
    for (const auto &cell : m_dof_handler.active_cell_iterators())
    {
      ...
      fe_values[m_d_fe].get_function_values(solution_old,
  ...
  double PhaseFieldMonolithicSolve<dim>::line_search_stepsize_strong_wolfe(
      const double phi_0, const double phi_0_prime,
      const BlockVector<double> &BFGS_p_vector,
      const BlockVector<double> &solution_delta)
  {
  ...
  template <int dim>
  double PhaseFieldMonolithicSolve<dim>::line_search_zoom_strong_wolfe(
      double phi_low, double phi_low_prime, double alpha_low, double phi_high,
      double phi_high_prime, double alpha_high, double phi_0, double phi_0_prime,
      const BlockVector<double> &BFGS_p_vector, double c1, double c2,
      unsigned int max_iter, const BlockVector<double> &solution_delta)
  {
    double alpha = 0;
    std::pair<double, double> current_phi_phi_prime;
    double phi, phi_prime;
  ...
  template <int dim>
  double PhaseFieldMonolithicSolve<dim>::line_search_interpolation_cubic(
      const double alpha_0, const double phi_0, const double phi_0_prime,
      const double alpha_1, const double phi_1, const double phi_1_prime)
  {
    const double d1 =
  ...
  template <int dim>
  std::pair<double, double>
  PhaseFieldMonolithicSolve<dim>::calculate_phi_and_phi_prime(
      const double alpha, const BlockVector<double> &BFGS_p_vector,
      const BlockVector<double> &solution_delta)
  {
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::LBFGS_B0(
      BlockVector<double> &LBFGS_r_vector, BlockVector<double> &LBFGS_q_vector)
  {
    ...
    if (m_parameters.m_type_linear_solver == "Direct")
    {
      ...
      A_direct.vmult(LBFGS_r_vector, LBFGS_q_vector);
    }
    else if (m_parameters.m_type_linear_solver == "CG")
    {
      ...
      */
      ...
      cg_uu.solve(m_tangent_matrix.block(m_u_dof, m_u_dof),
                  LBFGS_r_vector.block(m_u_dof), LBFGS_q_vector.block(m_u_dof),
                  preconditioner_uu);
      ...
      cg_dd.solve(m_tangent_matrix.block(m_d_dof, m_d_dof),
                  LBFGS_r_vector.block(m_d_dof), LBFGS_q_vector.block(m_d_dof),
                  preconditioner_dd);
    }
    else
    {
      AssertThrow(false, ExcMessage("Selected linear solver not implemented!"));
    }
  ...
  {
    m_logfile << "\t\t"
              << "L-BFGS (warning: without phasefield irreversibility)"
              << std::endl;
    ;
  ...
  void PhaseFieldMonolithicSolve<dim>::print_conv_header_LBFGSB()
  {
    ...
    m_logfile << "\t itr  "
              << " |    LB    UB    LUB    CG-itr    LS-alpha     Energy      "
                 "Res_Norm    "
              << " Res_u      Res_d    Inc_Norm   "
              << " Inc_u      Inc_d" << std::endl;
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::solve_nonlinear_timestep_LBFGS(
      BlockVector<double> &solution_delta,
      BlockVector<double> &LBFGS_update_refine)
  {
    ...
    std::list<
        std::pair<std::pair<BlockVector<double>, BlockVector<double>>, double>>
        LBFGS_vector_list;
         ...
         ++LBFGS_iteration)
    {
      ...
      else if (LBFGS_iteration == 1)
      {
        ...
        // mode if we use assemble_system_rhs_BFGS_parallel, then condense() is
        // not necessary
      ...
      */
      ...
      if (m_parameters.m_type_line_search == "StrongWolfe")
      {
        ...
        line_search_parameter = line_search_stepsize_strong_wolfe(
            phi_0, phi_0_prime, LBFGS_r_vector, solution_delta);
      }
      else if (m_parameters.m_type_line_search == "GradientBased")
      {
        // LBFGS_r_vector is the search direction
        line_search_parameter =
            line_search_stepsize_gradient_based(LBFGS_r_vector, solution_delta);
      }
      else
      {
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::calculate_cauchy_point(
      const BlockSparseMatrix<double> &B0_matrix,
      const std::list<BlockVector<double>> &y_vector_list,
      const std::list<BlockVector<double>> &b0xs_vector_list,
      const FullMatrix<double> &M_matrix, const BlockVector<double> &gradient_g,
      const BlockVector<double> &solution_delta,
      BlockVector<double> &solution_delta_cauchy_point)
  {
    ...
    std::priority_queue<std::pair<double, unsigned int>,
                        std::vector<std::pair<double, unsigned int>>,
                        std::greater<std::pair<double, unsigned int>>>
        t_series = calculate_break_points(solution_delta, gradient_g, gradient_d);

    // m_active_set_phasefield contains 1 or 2 for active set and 0 for inactive
    // set
    for (unsigned int i = 0; i < m_active_set_phasefield.size(); ++i)
    {
      if (m_active_set_phasefield(i) > 0.5)
        solution_delta_cauchy_point.block(m_d_dof)[i] =
            solution_delta.block(m_d_dof)[i];
    }
    ...
    while (delta_t_min >= delta_t)
    {
      ...
      if (gradient_d.block(m_d_dof)[b] > 0)
        solution_delta_cauchy_point.block(m_d_dof)[b] =
            1.0 - m_solution.block(m_d_dof)[b];
      else if (gradient_d.block(m_d_dof)[b] < 0)
        solution_delta_cauchy_point.block(m_d_dof)[b] = 0.0;
      else
        AssertThrow(false, ExcMessage("gradient_d(b) cannot be zero!"));
      ...
      if (list_size > 0)
      {
        ...
        f_prime_prime -= gradient_g.block(m_d_dof)[b] *
                         gradient_g.block(m_d_dof)[b] * (w_b_T_x_M * w_b);
      }
    ...
    (solution_delta_cauchy_point.block(m_u_dof))
        .add(t_old, gradient_d.block(m_u_dof));

    // We need to make sure the solution_delta_cauchy_point satisfies the
    // essential boundary conditions and the hanging-node constraints
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::solve_nonlinear_timestep_LBFGS_B(
      BlockVector<double> &solution_delta,
      BlockVector<double> &LBFGS_update_refine)
  {
    BlockVector<double> LBFGS_update(m_dofs_per_block);
    ...
    {
      if (m_parameters.m_output_iteration_history)
        m_logfile << '\t' << std::setw(4) << LBFGS_iteration << ' ' << std::flush;
      ...
      get_error_residual_LBFGSB(m_error_residual, solution_delta);
          ...
          lowerupper_bound_number_new == lowerupper_bound_number_old)
      {
        ...
        m_logfile << "\t\tThe current L-BFGS-B step converges in "
                  << LBFGS_iteration << " iterations." << std::endl;
        m_logfile << "\t\tNumber of active lower bounds not changed."
                  << std::endl;
        m_logfile << "\t\tNumber of active upper bounds not changed."
                  << std::endl;
        m_logfile << "\t\tNumber of active lower-upper bounds not changed."
                  << std::endl;
      ...
      // In the iteration LBFGS_iteration = 0, only the essential boundary
      // conditions are applied. WHen LBFGS_iteration = 1, it is the first step of
      // LBFGS update, and the s_vector_list and y_vector_list are empty. Since
      // the pair of s and y will only be added to the list if s dot y > tol, it
      // is safer to decide the matrix dimension by the size of the list.
      ...
      for (unsigned int i = 0; i < list_size; ++i)
        ...
        {
          sTxBxs_matrix(i, j) =
              (*std::next(itr_s_begin, i)) * (*std::next(itr_b0xs_begin, j));
        }
      for (unsigned int i = 0; i < list_size; ++i)
      ...
      for (unsigned int i = 0; i < list_size; ++i)
        for (unsigned int j = 0; j < i; ++j)
          L_matrix(i, j) =
              (*std::next(itr_s_begin, i)) * (*std::next(itr_y_begin, j));
      ...
      calculate_cauchy_point(m_tangent_matrix, y_vector_list, b0xs_vector_list,
                             M_matrix, m_system_rhs, solution_delta,
      ...
      for (unsigned int i = 0; i < list_size; ++i)
      {
        ...
        temp_vector_3(i + list_size) =
            (*std::next(itr_b0xs_begin, i)) * temp_vector_1;
      }
      ...
      for (unsigned int i = 0; i < list_size; ++i)
      {
        ...
        temp_vector_5.add(temp_vector_4(i + list_size),
                          (*std::next(itr_b0xs_begin, i)));
      }
      ...
      if (m_parameters.m_type_linear_solver == "CG")
      {
        ...
        const double cg_tol =
            m_parameters.m_CG_tolerace; // std::min( 0.1, std::sqrt(rc_hat_norm) )
                                        // * rc_hat_norm;
        ...
        if (list_size > 0)
        {
          ...
          for (auto itr = y_vector_list.begin(); itr != y_vector_list.end();
               ++itr)
          {
            zT_y_vector = (*itr);
            for (unsigned int i = 0; i < m_dofs_per_block[m_u_dof]; ++i)
          ...
          for (auto itr = b0xs_vector_list.begin(); itr != b0xs_vector_list.end();
               ++itr)
          {
            zT_b0xs_vector = (*itr);
            for (unsigned int i = 0; i < m_dofs_per_block[m_u_dof]; ++i)
          ...
          FullMatrix<double> zT_W_matrix_u(m_dofs_per_block[m_u_dof],
                                           2 * list_size);
          unsigned int j = 0;
          ...
          FullMatrix<double> zT_W_matrix_d(m_dofs_per_block[m_d_dof],
                                           2 * list_size);
          j = 0;
          for (auto itr = zT_y_list.begin(); itr != zT_y_list.end(); ++itr)
          {
          ...
          const auto op_uMuT = op_zT_W_matrix_u * op_M_matrix *
                               transpose_operator(op_zT_W_matrix_u);

          const auto op_uMdT = op_zT_W_matrix_u * op_M_matrix *
                               transpose_operator(op_zT_W_matrix_d);

          const auto op_dMuT = op_zT_W_matrix_d * op_M_matrix *
                               transpose_operator(op_zT_W_matrix_u);

          const auto op_dMdT = op_zT_W_matrix_d * op_M_matrix *
                               transpose_operator(op_zT_W_matrix_d);

          // const auto op_zT_wMwT_z = block_operator<2, 2,
          // BlockVector<double>>({op_uMuT, op_uMdT, 								     op_dMuT, op_dMdT});

          const std::array<
              std::array<dealii::LinearOperator<
                             dealii::Vector<double>, dealii::Vector<double>,
                             dealii::internal::LinearOperatorImplementation::
                                 EmptyPayload>,
                         2>,
              2>
              ops = {{{{op_uMuT, op_uMdT}}, {{op_dMuT, op_dMdT}}}};

          const auto op_zT_wMwT_z =
              block_operator<2, 2, BlockVector<double>>(ops);
          ...
          else if (m_parameters.m_type_preconditioner == "Jacobi")
          {
            ...
            const auto op_total_inv =
                inverse_operator(op_total, cg, preconditioner);
            op_total_inv.vmult(search_direction, rhs_vector);
          }
      ...
      else if (m_parameters.m_type_linear_solver == "Direct")
      {
        ...
        // zT_B0_z_inv_phasefield.initialize(m_tangent_matrix.block(m_d_dof,
        // m_d_dof));

        m_timer.leave_subsection();

        m_timer.enter_subsection(
            "Subspace direct solve (LU matrix-vector multiplication)");

        zT_B0_z_inv.vmult(search_direction, rhs_vector);
        // zT_B0_z_inv_disp.vmult(search_direction.block(m_u_dof),
        // rhs_vector.block(m_u_dof));
        // zT_B0_z_inv_phasefield.vmult(search_direction.block(m_d_dof),
        // rhs_vector.block(m_d_dof));
        ...
        if (list_size > 0)
        {
               ...
               ++itr)
          {
            ...
            // zT_B0_z_inv_disp.vmult(zT_B0_z_inv_zT_y_vector.block(m_u_dof),
            // zT_y_vector.block(m_u_dof));
            // zT_B0_z_inv_phasefield.vmult(zT_B0_z_inv_zT_y_vector.block(m_d_dof),
            // zT_y_vector.block(m_d_dof));
               ...
               ++itr)
          {
            ...
            // zT_B0_z_inv_disp.vmult(zT_B0_z_inv_zT_b0xs_vector.block(m_u_dof),
            // zT_b0xs_vector.block(m_u_dof));
            // zT_B0_z_inv_phasefield.vmult(zT_B0_z_inv_zT_b0xs_vector.block(m_d_dof),
            // zT_b0xs_vector.block(m_d_dof));
          ...
          const auto itr_zT_B0_z_inv_zT_y_list_begin =
              zT_B0_z_inv_zT_y_list.begin();
          const auto itr_zT_B0_z_inv_zT_b0xs_list_begin =
              zT_B0_z_inv_zT_b0xs_list.begin();
          for (unsigned int i = 0; i < list_size; ++i)
            for (unsigned int j = 0; j < list_size; ++j)
            {
              wT_z_zT_B0_z_inv_zT_w(i, j) =
                  (*std::next(itr_zT_y_list_begin, i)) *
                  (*std::next(itr_zT_B0_z_inv_zT_y_list_begin, j));

              wT_z_zT_B0_z_inv_zT_w(i, j + list_size) =
                  (*std::next(itr_zT_y_list_begin, i)) *
                  (*std::next(itr_zT_B0_z_inv_zT_b0xs_list_begin, j));

              wT_z_zT_B0_z_inv_zT_w(i + list_size, j) =
                  (*std::next(itr_zT_b0xs_list_begin, i)) *
                  (*std::next(itr_zT_B0_z_inv_zT_y_list_begin, j));

              wT_z_zT_B0_z_inv_zT_w(i + list_size, j + list_size) =
                  (*std::next(itr_zT_b0xs_list_begin, i)) *
                  (*std::next(itr_zT_B0_z_inv_zT_b0xs_list_begin, j));
            }
          ...
          for (unsigned int i = 0; i < list_size; ++i)
          {
            wT_z_zT_B0_z_inv_rhs(i) =
                (*std::next(itr_zT_B0_z_inv_zT_y_list_begin, i)) * rhs_vector;
            wT_z_zT_B0_z_inv_rhs(i + list_size) =
                (*std::next(itr_zT_B0_z_inv_zT_b0xs_list_begin, i)) * rhs_vector;
          }
          ...
          for (auto itr = zT_B0_z_inv_zT_y_list.begin();
               itr != zT_B0_z_inv_zT_y_list.end(); ++itr)
          {
            update_vector.add(middle_matrix_wT_z_zT_B0_z_inv_rhs(index), *itr);
            ++index;
          }
          for (auto itr = zT_B0_z_inv_zT_b0xs_list.begin();
               itr != zT_B0_z_inv_zT_b0xs_list.end(); ++itr)
          {
            update_vector.add(middle_matrix_wT_z_zT_B0_z_inv_rhs(index), *itr);
            ++index;
          }
      ...
      LBFGS_update.block(m_u_dof) = solution_delta_cauchy_point.block(m_u_dof) +
                                    search_direction.block(m_u_dof) -
                                    solution_delta.block(m_u_dof);

      for (unsigned int i = 0; i < m_dofs_per_block[m_d_dof]; ++i)
      {
        // phasefield active constraints
        if (m_active_set_phasefield(i) > 0.5)
          LBFGS_update.block(m_d_dof)[i] =
              solution_delta_cauchy_point.block(m_d_dof)[i] -
              solution_delta.block(m_d_dof)[i];
        else
          LBFGS_update.block(m_d_dof)[i] =
      ...
      /*
         ...
         LBFGS_update.block(m_d_dof)[i] < 0.0) LBFGS_update.block(m_d_dof)[i] =
         -solution_delta.block(m_d_dof)[i];
      ...
      */
      ...
      if (m_parameters.m_type_line_search == "StrongWolfe")
      {
        ...
        line_search_parameter = line_search_stepsize_strong_wolfe(
            phi_0, phi_0_prime, LBFGS_update, solution_delta);
      }
      else if (m_parameters.m_type_line_search == "GradientBased")
      {
        // LBFGS_update is the search direction
        line_search_parameter =
            line_search_stepsize_gradient_based(LBFGS_update, solution_delta);
      }
      else
      {
        Assert(false, ExcMessage("An unknown line search method is called!"));
      ...
      for (unsigned int i = 0; i < m_dofs_per_block[m_d_dof]; ++i)
      {
        if (solution_delta.block(m_d_dof)[i] + LBFGS_update.block(m_d_dof)[i] <
            0.0)
          LBFGS_update.block(m_d_dof)[i] = -solution_delta.block(m_d_dof)[i];

        if (solution_delta.block(m_d_dof)[i] + m_solution.block(m_d_dof)[i] +
                LBFGS_update.block(m_d_dof)[i] >
            1.0)
      ...
      const double compare_tol = 1.0e-9;
      for (unsigned int i = 0; i < m_dofs_per_block[m_d_dof]; ++i)
      {
        // We should avoid to directly compare two float values
        if (std::fabs(solution_delta.block(m_d_dof)[i] - 0.0) < compare_tol &&
            std::fabs(solution_phasefield_total[i] - 1.0) < compare_tol)
        {
          m_active_set_phasefield(i) = 3; // lower bound overlaps with upper bound
          ++number_active_constraint_lowerupper_bound;
        }
        else if (std::fabs(solution_delta.block(m_d_dof)[i] - 0.0) <
                     compare_tol &&
                 std::fabs(solution_phasefield_total[i] - 1.0) >= compare_tol)
        {
          m_active_set_phasefield(i) = 1; // lower bound
      ...
      if (m_parameters.m_output_iteration_history)
      {
        ...
        m_logfile << "  | " << std::setw(6)
                  << number_active_constraint_lower_bound << std::setw(6)
                  << number_active_constraint_upper_bound << std::setw(6)
                  << number_active_constraint_lowerupper_bound;
        if (m_parameters.m_type_linear_solver == "CG")
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::output_results() const
  {
    ...
    data_out.add_data_vector(m_solution, solution_name,
    ...
    data_out.add_data_vector(active_set_status, solution_name_active_set,
                             DataOut<dim>::type_dof_data,
                             data_component_interpretation_active_set);
  ...
  template <int dim>
  std::pair<double, double> PhaseFieldMonolithicSolve<
  ...
  void PhaseFieldMonolithicSolve<dim>::print_parameter_information()
  {
    ...
    if (m_parameters.m_type_linear_solver == "CG")
    {
      m_logfile << "Preconditioner type for CG = "
                << m_parameters.m_type_preconditioner << std::endl;
      m_logfile << "Convergence tolerance for CG iterations = "
                << m_parameters.m_CG_tolerace << std::endl;
    }
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::run()
  {
